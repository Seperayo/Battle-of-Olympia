...
void frecruit(Player * P1, Player * P2, MATRIKS *M, TurnQueue *Q) {
    ...
    if ((PlayerID==1)&&(Elmt(*M,B-2,1).UNITINFO.TYP == KNG)&&(!IsCastleFull(PlayerID,*M))) {
    ...
    } else if ((PlayerID==2)&&(Elmt(*M,1,K-2).UNITINFO.TYP == KNG)&&(!IsCastleFull(PlayerID,*M))) {
        printf("Enter the coordinate of empty castle = ");
        scanf("%d %d",&x,&y);
...
int fnext_unit(Player P) {
    int x;
		addressunit K;
		boolean found=false;
    printf("\n====== LIST OF UNITS ======\n");
    PrintInfo((P).UNTLST);
		K=FirstUnit((P).UNTLST);
		x=1;
		while(x<=NbElmt((P).UNTLST) && found){
			 if((InfoUnit(K).MOVEPTS)>0){
				 found=true;
			 } else{x=x+1; K=NextUnit(K);}
		};
		if(found==true){
    	return x;
		}else{
