...
void Menu(){
	Kata pilihan;
	Kata start,load,save,exit,move,undo,recruit,change_unit,next_unit,attack,map,info,end_turn;
	int check;
	...
	while (!IsKataSama(exit,pilihan)) {
		while (!IsKataSama(end_turn,pilihan)) {
			...
			if (IsKataSama(move,pilihan)) {
				...
				printf("\n");
				finfo(M);
			} else if (IsKataSama(next_unit,pilihan)) {
				if (CTurn == 1) {
						check=fnext_unit(P1);
						if(check==0){printf("You don't have any units that can move!");}
						else{
								CreateEmptyS(&S1);
								CurrUnit1 = SearchX((P1).UNTLST,check);
								Push(&S1,CurrUnit1);
								DelP(&(P1.UNTLST), CurrUnit1);
								InsVFirst(&P1.UNTLST, CurrUnit1);
								}
						...
						if(check==0){printf("You don't have any units that can move!");}
								...
								CreateEmptyS(&S2);
								CurrUnit2 = SearchX((P2).UNTLST,check);
								Push(&S2,CurrUnit2);
								DelP(&(P2.UNTLST), CurrUnit2);
								InsVFirst(&P2.UNTLST, CurrUnit2);
								}
