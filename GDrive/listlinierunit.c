...
boolean IsUnitSama(Unit X, Unit Y) {
    if ( (X.MAXHP == Y.MAXHP) && (X.HP == Y.HP) &&  (X.ATK == Y.ATK) && (X.MAXMOVE==Y.MAXMOVE) && (X.MOVEPTS == Y.MOVEPTS) && (X.PRC==Y.PRC) && (X.RNG==Y.RNG) && (X.RNGTYP==Y.RNGTYP) && (X.TYP==Y.TYP) && (X.CHN==Y.CHN) && (X.P==Y.P) && (X.LOC.X==Y.LOC.X) && (X.LOC.Y==Y.LOC.Y) ) {
        return true;
    } else {
        return false;
    }
