...
/* Jika alokasi gagal, mengirimkan Nil */
{
	...
	if (P!=Nil) {
        InfoUnit(P)->MAXHP = X.MAXHP;
        InfoUnit(P)->HP = X.HP;
        InfoUnit(P)->ATK = X.ATK;
        InfoUnit(P)->MAXMOVE = X.MAXMOVE;
        InfoUnit(P)->MOVEPTS = X.MOVEPTS;
        InfoUnit(P)->PRC = X.PRC;
        InfoUnit(P)->RNG = X.RNG;
        InfoUnit(P)->RNGTYP = X.RNGTYP;
        InfoUnit(P)->TYP = X.TYP;
        InfoUnit(P)->CHN = X.CHN;
        InfoUnit(P)->P.ID = X.P.ID;
        /*
        if (X=='K') {
            InfoUnit(P)->MAXHP = MAX_HP_KNG;
            InfoUnit(P)->HP = MAX_HP_KNG;
            InfoUnit(P)->ATK = ATK_KNG;
            InfoUnit(P)->MAXMOVE = MAX_MOVE_KNG;
            InfoUnit(P)->MOVEPTS = MAX_MOVE_KNG;
            InfoUnit(P)->PRC = 0;
            InfoUnit(P)->RNG = 1;
            InfoUnit(P)->RNGTYP = MELEE;
            InfoUnit(P)->TYP = KNG;
            InfoUnit(P)->CHN = CHN_KNG;
        } else if (X=='A') {
            InfoUnit(P)->MAXHP = 
            InfoUnit(P)->HP = MAX_HP_ARC;
            InfoUnit(P)->ATK = ATK_ARC;
            InfoUnit(P)->MAXMOVE = MAX_MOVE_ARC;
            InfoUnit(P)->MOVEPTS = MAX_MOVE_ARC;
            InfoUnit(P)->PRC = PRC_OF_ARC;
            InfoUnit(P)->RNG = 1;
            InfoUnit(P)->RNGTYP = RANGE;
            InfoUnit(P)->TYP = ARC;
            InfoUnit(P)->CHN = CHN_ARC;
        } else if (X=='S') {
            InfoUnit(P)->MAXHP = MAX_HP_SWD;
            InfoUnit(P)->HP = MAX_HP_SWD;
            InfoUnit(P)->ATK = ATK_SWD;
            InfoUnit(P)->MAXMOVE = MAX_MOVE_SWD;
            InfoUnit(P)->MOVEPTS = MAX_MOVE_SWD;
            InfoUnit(P)->PRC = PRC_OF_SWD;
            InfoUnit(P)->RNG = 1;
            InfoUnit(P)->RNGTYP = MELEE;
            InfoUnit(P)->TYP = SWD;
            InfoUnit(P)->CHN = CHN_SWD;
        } else if (X=='W') {
            InfoUnit(P)->MAXHP = MAX_HP_WHT;
            InfoUnit(P)->HP = MAX_HP_WHT;
            InfoUnit(P)->ATK = ATK_WHT;
            InfoUnit(P)->MAXMOVE = MAX_MOVE_WHT;
            InfoUnit(P)->MOVEPTS = MAX_MOVE_WHT;
            InfoUnit(P)->PRC = PRC_OF_WHT;
            InfoUnit(P)->RNG = 1;
            InfoUnit(P)->RNGTYP = RANGE;
            InfoUnit(P)->TYP = WHT;
            InfoUnit(P)->CHN = CHN_WHT;
        }
        InfoUnit(P)->P.ID = PI;
        NextUnit(P)=Nil;
        */
...
void PrintInfo (ListUnit L)
{
	...
	for (i=1;i<=NbElmt(L);i++) {
        printf("%d. ",k);
        if ( InfoUnit(P)->TYP == KNG ) {
            printf("King");
        } else if ( InfoUnit(P)->TYP == ARC ) {
            printf("Archer");
        } else if ( InfoUnit(P)->TYP == SWD ) {
            printf("Swordsman");
        } else if ( InfoUnit(P)->TYP == WHT ) {
            printf("White Mage");
        }
...
boolean IsUnitSama(Unit *X, Unit Y) {
    if ( (X->MAXHP == Y.MAXHP) && (X->HP == Y.HP) &&  (X->ATK == Y.ATK) && (X->MAXMOVE==Y.MAXMOVE) && (X->MOVEPTS == Y.MOVEPTS) && (X->PRC==Y.PRC) && (X->RNG==Y.RNG) && (X->RNGTYP==Y.RNGTYP) && (X->TYP==Y.TYP) && (X->CHN==Y.CHN) && (X->P.ID==Y.P.ID) && (X->LOC.X==Y.LOC.X) && (X->LOC.Y==Y.LOC.Y) ) {
        return true;
    } else {
        return false;
    }
