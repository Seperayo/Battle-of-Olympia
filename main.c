...
void LoadGame(Player *P1, Player *P2, addressunit *X, Unit *CurrUnit1, Unit *CurrUnit2, ListUnit *LU1, ListUnit *LU2, Stack *S, TurnQueue *Q, MATRIKS *M){
	LSTARTKATA();
	int B,K,i,j;
	Unit P;
	BLD V;
...
void NewGame(Player *P1, Player *P2, addressunit *X, Unit *CurrUnit1, Unit *CurrUnit2, ListUnit *LU1, ListUnit *LU2, Stack *S, TurnQueue *Q, MATRIKS *M){
...
void StartGame(Player P1, Player P2, addressunit X, Unit CurrUnit1, Unit CurrUnit2, ListUnit LU1, ListUnit LU2, Stack S, TurnQueue Q, MATRIKS M){
	...
	while (!IsKataSama(menu,pilihan)) {
		while (!IsKataSama(end_turn,pilihan) && !IsKataSama(menu,pilihan)) {
			...
			if (IsKataSama(move,pilihan)) {
				...
				printf("\n");
				SaveGame(P1,P2,X,CurrUnit1,CurrUnit2,LU1,LU2,S,Q,M);
			} 
...
int main(){
	...
	do{
		...
		Menu(&option);
		if(option == 1){
			NewGame(&P1,&P2,&X,&CurrUnit1,&CurrUnit2,&LU1,&LU2,&S,&Q,&M);
			StartGame(P1,P2,X,CurrUnit1,CurrUnit2,LU1,LU2,S,Q,M);
		}
		else if(option == 2){
			LoadGame(&P1,&P2,&X,&CurrUnit1,&CurrUnit2,&LU1,&LU2,&S,&Q,&M);
			StartGame(P1,P2,X,CurrUnit1,CurrUnit2,LU1,LU2,S,Q,M);
			char junk;
			scanf("%c",&junk);
		}
